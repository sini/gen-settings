...
- No hashing — `git grep -n -E 'hashString|builtins\.hash|hashFile' -- lib/` returns nothing; positive control `git grep -c -E 'id_hash' -- lib/` hits 7 files. Relatum identities are read, never minted; the `attaches` binding stamp IS minted, but by calling gen-schema's `hashIdentity` — no digest is computed here.
