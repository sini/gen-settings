...
let
  ...
  # lock, whose direct edges agree with the `lock.nodes.<label>` shortcut on `locked.repo` even where
  # they resolve to a different NODE — so it now drives the SHIM's binding: with that shortcut
  # written into `default.nix`'s `resolve`, the control below reds.
...
in
{
  ...
  flake.tests.entry.test-standalone-entry-matches-lib = {
    ...
    expected = builtins.attrNames genSettings;
  };
  ...
  # fold in `default.nix` with the shortcut and this reds; the cell above cannot be relied on to: it
  # reads `locked.repo`, and on this library's own lock the two rules agree on `locked.repo` at the
  # `prelude` edge (`gen-prelude` root input → node `gen-prelude_3`) even while disagreeing on the
  # NODE — so it reds at none of the tampered arms the control below catches.
